...
def create_auth_api_client(
    *,
    ...
    return AuthenticatedClient(
        ...
        credentials=credentials,
        token_refreshing_endpoint=_default_token_refreshing_endpoint(credentials.base_url),
        api_key_exchange_callback=_exchange_api_key_with_mapped_errors,
...
def _default_token_refreshing_endpoint(base_url: str) -> str:
    return f"{base_url.rstrip('/')}/auth/realms/neptune/protocol/openid-connect/token"


def _exchange_api_key_with_mapped_errors(client: AuthenticatedClient | Client, credentials: Credentials) -> OAuthToken:
    try:
        return exchange_api_key(client=client, credentials=credentials)
