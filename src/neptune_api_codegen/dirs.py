...
def find_project_root() -> Path:
    ...
    for parent in [current_dir] + list(current_dir.parents):
        if (parent / "pyproject.toml").exists():
            return parent
    raise RuntimeError("Could not find the project root (directory containing pyproject.toml).")
...
def find_neptune_repo(search_start: Path, up_levels: int = 2, down_levels: int = 4, limit_at_level: int = 128) -> Path | None:
    candidate_dirs = set()
