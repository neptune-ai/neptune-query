    ...
    try:
        ...
        if path_stat.st_uid != uid or path_stat.st_gid != gid:
            ...
            if not updated_parent and not updated_grandparent:
                print(
                    f"Fixing ownership of {path} from {path_stat.st_uid}:{path_stat.st_gid} to {uid}:{gid}",
                    file=sys.stderr,
                )
...
def fix_file_ownership_in_dir(path: Path, ref_path: Path, recurse: bool = False) -> None:
    """
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print("Usage: chown.py [-R] <target_directory> <reference_path>", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        fix_file_ownership_in_dir(target_directory, reference_path, recurse=recurse)
    except Exception as e:
