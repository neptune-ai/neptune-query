...
from neptune_query.internal.identifiers import RunAttributeDefinition, RunIdentifier, AttributeDefinition, SysId, \
    ProjectIdentifier
...
from neptune_query.internal.retrieval.metrics import FloatPointValue, StepIndex
...
def metric_datasets(draw) -> Tuple[dict[RunAttributeDefinition, list[FloatPointValue]], dict[SysId, str]]:
    ...
    run_attribute_definition_list = draw(st.lists(run_attribute_definitions(project_identifier=project_identifier, types=["float_series"])))
    metrics_data = {
    ...
    labels = draw(st.lists(
        ...
        min_size=len(run_attribute_definition_list), max_size=len(run_attribute_definition_list), unique=True)
    )
    label_mapping = {
...
@st.composite
def run_attribute_definitions(draw, *, project_identifier: ProjectIdentifier, types: Sequence[str] = ALL_TYPES) -> RunAttributeDefinition:
...
def project_identifiers(draw) -> RunIdentifier:
    organization = draw(st.text(alphabet=st.characters(codec="utf-8", exclude_characters="/"), min_size=1, max_size=255))
...
def test_create_metrics_dataframe(
    ...
    # validate index
    expected_experiment_steps = sorted(list({
    ...
    # validate columns
    expected_attributes = sorted(list({
       f"{run_attributes.attribute_definition.name}:{run_attributes.attribute_definition.type}" if type_suffix_in_column_names else run_attributes.attribute_definition.name
        for run_attributes, points in metrics_data.items()
        if points
    }))
